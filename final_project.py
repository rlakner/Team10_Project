...
class Item:
    ...
    def __init__(self, name, price, condition, link, score = 0):
        ...
        Args:
            ...
            condition(str): the condition of the item 
            link(str): the unique direct link to the item on Depop
            score(float): a score representing how good of a deal the item is on 
                a scale of from 1 to 5
        """
        self.name = name
        self.price = price
...
class DepopScraper:
    """A class for scraping data for items listed on Depop
    ...
    def scrape_items(search):
        ...
        Args:
        search(string): The item a user is searching for which will be used to 
            add to the Depop base URL 

        Returns: 
            items(list): a list of dictionaries of the items found with the key 
            as the name of the item and the value being a tuple containaint its 
    ...
    def scrape_images(items):
        ...
        Args:
        items(list): The list of dictionaries returned by scrape_items which 
            will then be parsed to extract the unique direct links

        Returns: 
            images(list): A list of dictionaries of the item images to be 
            displayed in the user interface, having the links as keys and the 
            image source links
        """
    ...
    def averages(new_items_prices, used_items_prices):
        ...
        Args:
            new_items_prices(list): a list of floats the represent the price of 
                items that result from a search that are either listed as 'New' 
                or 'Like New' 
            used_items(list): a list of floats the represent the price of 
                items that result from a search that are listed as any variation 
                of 'Used'
        Returns:
    ...
    def get_max_price(prices):
        ...
        Args:
            prices(list): a list of floats the represent the price of listed 
            items that result from a search
    ...
    def sort_by_price(items, ascending):
        ...
        Args:
            items(list): a list of dictionaries of the items found with the key 
                as the name of the item and the value being a tuple containaint 
                its price, condition, and unique direct link  
    ...
    def best_match(new_items, used_items, new_price, used_price, avg_price):
        ...
        Args:
            ...
            avg_price(float): the average price of all listed items from a 
                search, regardless of condition
